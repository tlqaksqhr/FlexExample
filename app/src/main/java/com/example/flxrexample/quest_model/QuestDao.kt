...
interface QuestDao {
    ...
    @Query("SELECT quest_table.title, quest_table.`desc`, quest_table.isCompleted, quest_table.challengingCount, quest_table.totalStar ,quest_table.questStar, quest_table.numOfComplete, quest_table.address, quest_table.latLng, quest_table.startDate, quest_table.endDate, quest_table.id, quest_constraint_table.constraintNum, quest_constraint_table.content, quest_constraint_table.pictureURL FROM quest_table JOIN quest_constraint_table ON quest_table.id = quest_constraint_table.quest_id AND quest_table.id = :id")
