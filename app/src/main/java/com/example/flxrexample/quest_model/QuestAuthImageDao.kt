...
interface QuestAuthImageDao {
    ...
    @Query("SELECT quest_auth_image_table.pictureURL, quest_auth_image_table.predicts, quest_auth_image_table.quest_constraint_id, quest_auth_image_table.id FROM quest_auth_image_table INNER JOIN quest_table, quest_constraint_table ON quest_constraint_table.id = quest_auth_image_table.quest_constraint_id AND quest_constraint_table.quest_id = quest_table.id WHERE quest_table.id = :id")
